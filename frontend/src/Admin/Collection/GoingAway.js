...
const GoingAway = ()=>{
    ...
    const likes = -1;
    const unLikes = -1;
    const category = "goingAway";
    const [loading, setLoading] = useState(false); //additional 
    const [isError, setIsError] = useState(false);

    const goingAwayHandler = async (e)=>{
        e.preventDefault();
        ...
        try {
            const {data} = await axios.post("http://localhost:8070/goingAway/add" , {coupleName , imgURL1 , imgURL2 , imgURL3 , collectionURL , downloads , likes , unLikes , category, password} , config);
            toast("Success! GoingAway collectoin Added 😘")
            setLoading(false);

        } catch (error) {
            toast("Error! GoingAway Collection not Added Duplicate Key Found: Couple Name must be unique")
            setLoading(false);
            setIsError(true);
            setTimeout(()=>{
    ...
    return(
        <div>
            <form onSubmit={goingAwayHandler}>
                    <div className="card">
